...
// generated this with AI lol, it's kind of poop but i was curious how well it could implement my ideas
// ============================================  
// ADVANCED BOT AI CONTROLLER  
// ============================================  
  
// Bot personality types that influence all behaviors  
const BotPersonalities = {  
    AGGRESSIVE: {  
        ...
        dodgeDelay: 50,  
        chaseRange: 1.3,  
        buildPresets: { glassCannonChance: 0.6, rammerChance: 0.2, balancedChance: 0.2 },  
        sidestepping: { frequency: 0.7, distance: 1.2 },  
        mistakes: 0.005,  
        sillyBehavior: 0,//0.02,  
        aimSpeed: 0.15,  
        aimAccuracy: 0.92,  
    },  
    DEFENSIVE: {  
        ...
        dodgeDelay: 30,  
        chaseRange: 0.9,  
        buildPresets: { glassCannonChance: 0.1, rammerChance: 0.1, balancedChance: 0.8 },  
        sidestepping: { frequency: 0.9, distance: 1.5 },  
        mistakes: 0.008,  
        sillyBehavior: 0,//0.01,  
        aimSpeed: 0.12,  
        aimAccuracy: 0.88,  
    },  
    BALANCED: {  
        ...
        dodgeDelay: 40,  
        chaseRange: 1.0,  
        buildPresets: { glassCannonChance: 0.3, rammerChance: 0.2, balancedChance: 0.5 },  
        sidestepping: { frequency: 0.8, distance: 1.3 },  
        mistakes: 0.01,  
        sillyBehavior: 0,//0.03,  
        aimSpeed: 0.13,  
        aimAccuracy: 0.85,  
    },  
    CHAOTIC: {  
        ...
        dodgeDelay: 60,  
        chaseRange: 1.1,  
        buildPresets: { glassCannonChance: 0.4, rammerChance: 0.4, balancedChance: 0.2 },  
        sidestepping: { frequency: 0.6, distance: 1.0 },  
        mistakes: 0.02,  
        sillyBehavior: 0,//0.08,  
        aimSpeed: 0.1,  
        aimAccuracy: 0.75,  
    },  
    SNIPER: {  
        ...
        dodgeDelay: 25,  
        chaseRange: 0.7,  
        buildPresets: { glassCannonChance: 0.7, rammerChance: 0.0, balancedChance: 0.3 },  
        sidestepping: { frequency: 0.95, distance: 1.8 },  
        mistakes: 0.003,  
        sillyBehavior: 0.01,  
        aimSpeed: 0.18,  
        aimAccuracy: 0.95,  
...
class io_advancedBotAI extends IO {  
    constructor(body, opts = {}) {  
        ...
        console.log(body.guns)
        // Personality selection  
        this.personality = opts.personality || ran.choose(Object.values(BotPersonalities));  
          
        // Memory system  
        this.memory = {  
            damageFrom: new Map(), // entity.id -> { total: number, lastHit: timestamp, type: string }  
            threatening: new Map(), // entity.id -> threat level  
            safeSpots: [],  
            lastDeath: null,  
        };  
        ...
        // Aiming system - simulated mouse cursor  
        this.cursor = {  
            x: 0,  
            y: 0,  
            velocityX: 0,  
            ...
            targetX: 0,  
            targetY: 0,  
            accuracy: this.personality.aimAccuracy,  
        };  
          
        // Movement system  
        this.movement = {  
            currentAngle: 0, // Restricted to 45-degree increments  
            holdKeyDuration: 0,  
            targetLocation: null,  
            sidestepAngle: 0,  
            sidestepTimer: 0,  
            recoilBoosting: false,  
        };  
        ...
        // Build system  
        this.build = {  
            preset: this.selectBuildPreset(),  
            plannedClass: null,  
            skillPriorities: [],  
        };  
          
        // Behavior flags  
        this.isSillyMode = false;  
        this.sillyModeTimer = 0;  
        this.mistakeActive = false;  
        this.mistakeTimer = 0;  
          
        // Initialize planned class upgrade path  
        this.planClassUpgrades();  
    }  
      
    selectBuildPreset() {  
        const presets = this.personality.buildPresets;  
        const roll = Math.random();  
          
        if (roll < presets.glassCannonChance) {  
            return {  
                name: "Glass Cannon",  
                stats: [0, 0, 0, 7, 7, 7, 7, 0, 0, 0], // Focus on bullet stats  
                exactFollow: Math.random() < 0.3,  
            };  
        } else if (roll < presets.glassCannonChance + presets.rammerChance) {  
            return {  
                name: "Rammer",  
                stats: [0, 0, 0, 5, 5, 0, 7, 7, 7, 6], // Focus on body damage, health, speed  
                exactFollow: Math.random() < 0.3,  
            };  
        } else {  
            return {  
                name: "Balanced",  
                stats: [1, 1, 1, 4, 4, 4, 4, 4, 4, 4], // Balanced distribution  
                exactFollow: Math.random() < 0.5,  
            };  
        }  
    }  
      
    planClassUpgrades() {  
        // This would need to integrate with the tank upgrade system  
        // For now, store preferences based on personality  
        const body = this.body;  
        this.build.plannedClass = null; // Will be selected based on available upgrades  
    }  
      
    think(input) {  
        this.tick++;  
          
        // Random silly behavior  
        if (Math.random() < this.personality.sillyBehavior && !this.isSillyMode) {  
            this.activateSillyMode();  
        }  
          
        // Random mistakes  
        if (Math.random() < this.personality.mistakes && !this.mistakeActive) {  
            this.activateMistake();  
        }  
        ...
        // 1. Threat detection and avoidance  
        this.detectThreats();  
        ...
        // 3. Aiming system  
        if (this.currentTarget && !this.isSillyMode) {  
        ...
        // 4. Movement system  
        const movementResult = this.calculateMovement(input);  
        output.goal = movementResult.goal;  
        output.power = movementResult.power;  
          
        // 5. Firing logic  
        if (this.currentTarget && !this.mistakeActive) {  
            const dist = util.getDistance(this.body, this.currentTarget);  
        ...
        // 6. Recoil boosting when safe  
        if (this.shouldRecoilBoost() && !this.currentTarget) {  
            output.target = this.getRecoilBoostDirection();  
            output.fire = true;  
            output.main = true;  
        }  
          
        // 7. Silly mode overrides  
        if (this.isSillyMode) {  
            return this.sillyBehavior();  
        }  
    ...
    detectThreats() {  
        if (this.tick % 3 !== 0) return; // Check every 3 ticks  
          
        this.avoidance.threats = [];  
        const body = this.body;  
        const checkRadius = body.size * 15;  
          
        // Scan for bullets, drones, traps  
        for (const entity of entities) {  
            if (!entity || !entity.master || !entity.master.master || entity.master.master.team === body.team) continue;  
            if (!entity.isAlive()) continue;  
              
            const dist = util.getDistance(body, entity);  
            if (dist > checkRadius) continue;  
              
            if (entity.type === 'bullet' || entity.type === 'drone' ||   
                entity.type === 'swarm' || entity.type === 'trap') {  
                  
                // Calculate threat level based on velocity and distance  
                const threat = {  
                    entity: entity,  
                    distance: dist,  
                    danger: this.calculateThreatDanger(entity, dist),  
                    velocity: entity.velocity,  
                };  
                  
                this.avoidance.threats.push(threat);  
            }  
        }  
          
        // Sort by danger level  
        this.avoidance.threats.sort((a, b) => b.danger - a.danger);  
    }  
      
    calculateThreatDanger(entity, distance) {  
        let danger = 100 / (distance + 1);  
          
        // Factor in velocity toward bot  
        const velocityToward = this.getVelocityToward(entity, this.body);  
        if (velocityToward > 0) {  
            danger *= (1 + velocityToward);  
        }  
          
        // Factor in entity damage  
        danger *= (entity.DAMAGE || 1);  
          
        return danger;  
    }  
      
    getVelocityToward(entity, target) {  
    ...
    selectTarget() {  
        ...
        const body = this.body;  
        const range = this.getEffectiveRange() * 1.5;  
        this.validTargets = [];  
          
        // Build target list  
        for (const entity of targetableEntities.values()) {  
            if (!this.isValidTarget(entity, range)) continue;  
              
            const priority = this.calculateTargetPriority(entity);  
            this.validTargets.push({ entity, priority });  
        }  
          
        // Sort by priority (damage taken, low health, etc.)  
        this.validTargets.sort((a, b) => b.priority - a.priority);  
          
        if (this.validTargets.length > 0) {  
            const newTarget = this.validTargets[0].entity;  
            if (this.currentTarget !== newTarget) {  
                this.currentTarget = newTarget;  
                this.targetSwitchCooldown = 30; // Wait before switching again  
            }  
        } else {  
            this.currentTarget = null;  
        }  
    ...
    isValidTarget(entity, range) {  
        const body = this.body;  
        const theirMaster = entity.master.master;  
          
        if (entity.health.amount <= 0) return false;  
        if (theirMaster.team === body.team) return false;  
        if (entity.invuln || entity.godmode) return false;  
        if (theirMaster.passive || body.master.master.passive) return false;  
    ...
    calculateTargetPriority(entity) {  
        let priority = 0;  
          
        // Check memory for damage taken  
        const memoryData = this.memory.damageFrom.get(entity.id);  
        if (memoryData) {  
            priority += memoryData.total * 10;  
              
            // Recent damage is more important  
            const timeSince = Date.now() - memoryData.lastHit;  
            if (timeSince < 5000) {  
                priority += 50;  
            }  
        }  
        ...
        if (entity.health.amount < entity.health.max * 0.5) {  
            priority += 30;  
        }  
        if (entity.health.amount < entity.health.max * 0.3) {  
            priority += 50;  
        }  
          
        // Distance factor (closer = higher priority)  
        const dist = util.getDistance(this.body, entity);  
        priority += 100 / (dist + 1);  
    ...
    updateCursorAim() {  
        ...
        // Calculate desired aim position with leading  
        const tracking = this.getTrackingSpeed();  
        const lead = this.calculateLead(target, tracking);  
          
        const desiredX = target.x - body.x + lead.x;  
        const desiredY = target.y - body.y + lead.y;  
          
        // Add accuracy variance  
        const accuracyOffset = (1 - this.cursor.accuracy) * 50;  
        this.cursor.targetX = desiredX + (Math.random() - 0.5) * accuracyOffset;  
        this.cursor.targetY = desiredY + (Math.random() - 0.5) * accuracyOffset;  
          
        // Smoothly move cursor toward target (simulated mouse movement)  
        const aimSpeed = this.personality.aimSpeed;  
        const dx = this.cursor.targetX - this.cursor.x;  
        const dy = this.cursor.targetY - this.cursor.y;  
          
        // Velocity-based smooth movement  
        this.cursor.velocityX += dx * aimSpeed;  
        this.cursor.velocityY += dy * aimSpeed;  
          
        // Damping  
        this.cursor.velocityX *= 0.7;  
        this.cursor.velocityY *= 0.7;  
          
        this.cursor.x += this.cursor.velocityX;  
        this.cursor.y += this.cursor.velocityY;  
    }  
      
    calculateLead(target, tracking) {  
        const diff = {  
            x: target.x - this.body.x,  
            y: target.y - this.body.y,  
        };  
          
        const toi = timeOfImpact(diff, target.velocity, tracking);  
        ...
        return {  
            x: toi * target.velocity.x,  
            y: toi * target.velocity.y,  
        };  
    }  
    ...
    calculateMovement(input) {  
        ...
        if (this.avoidance.threats.length > 0) {  
            const avoidGoal = this.calculateAvoidanceGoal();  
            if (avoidGoal) {  
                goal = this.compressTo45Degrees(avoidGoal);  
                power = 1;  
                return { goal, power };  
            }  
        }  
        ...
        if (body.health.amount < body.health.max * this.personality.fleeThreshold) {  
            goal = this.calculateFleeGoal();  
            if (goal) {  
                goal = this.compressTo45Degrees(goal);  
                power = 1;  
                return { goal, power };  
            }  
        }  
          
        // Priority 3: Combat movement  
        if (this.currentTarget) {  
            goal = this.calculateCombatMovement();  
            goal = this.compressTo45Degrees(goal);  
            power = 0.8;  
            return { goal, power };  
        }  
          
        // Priority 4: Wander and look for targets  
        if (!goal) {  
            goal = this.calculateWanderGoal();  
            goal = this.compressTo45Degrees(goal);  
            power = 0.6;  
        }  
    ...
    compressTo45Degrees(goal) {  
        if (!goal) return null;  
          
        const body = this.body;  
        const dx = goal.x - body.x;  
        const dy = goal.y - body.y;  
        const angle = Math.atan2(dy, dx);  
          
        // Round to nearest 45 degrees  
        const angleOptions = [0, Math.PI/4, Math.PI/2, 3*Math.PI/4, Math.PI, -3*Math.PI/4, -Math.PI/2, -Math.PI/4];  
        let closest = angleOptions[0];  
        let minDiff = Math.abs(angle - closest);  
          
        for (const option of angleOptions) {  
            const diff = Math.abs(angle - option);  
            if (diff < minDiff) {  
                minDiff = diff;  
                closest = option;  
            }  
        ...
        // Hold key for realistic duration  
        if (Math.abs(closest - this.movement.currentAngle) < 0.1) {  
            this.movement.holdKeyDuration++;  
        } else {  
            this.movement.currentAngle = closest;  
            this.movement.holdKeyDuration = 0;  
        }  
          
        // Occasionally adjust slightly  
        if (this.movement.holdKeyDuration > 60 && Math.random() < 0.1) {  
            const adjacent = angleOptions[(angleOptions.indexOf(closest) + (Math.random() < 0.5 ? 1 : -1) + 8) % 8];  
            this.movement.currentAngle = adjacent;  
            this.movement.holdKeyDuration = 0;  
        }  
          
        return {  
            x: body.x + Math.cos(this.movement.currentAngle) * 10,  
            y: body.y + Math.sin(this.movement.currentAngle) * 10,  
        };  
    }  
      
    calculateAvoidanceGoal() {  
        if (this.avoidance.threats.length === 0) return null;  
          
        const body = this.body;  
        const threat = this.avoidance.threats[0]; // Most dangerous  
          
        // Move perpendicular to threat trajectory  
        const dx = threat.entity.x - body.x;  
        const dy = threat.entity.y - body.y;  
        const dist = Math.sqrt(dx * dx + dy * dy);  
        ...
        // Choose direction that moves away fastest  
        const vx = threat.velocity.x;  
        const vy = threat.velocity.y;  
        const dot = perpX * vx + perpY * vy;  
          
        const mult = dot > 0 ? -1 : 1;  
          
        return {  
            x: body.x + perpX * mult * 100,  
            y: body.y + perpY * mult * 100,  
        };  
    }  
      
    calculateFleeGoal() {  
        // Try to move toward spawn or safe areas  
        const body = this.body;  
        const room = global.gameManager.room;  
          
        // Head toward center (spawn area)  
        return {  
            x: room.width / 2,  
            y: room.height / 2,  
        };  
    ...
    calculateCombatMovement() {  
        const body = this.body;  
        const target = this.currentTarget;  
          
        if (!target) return null;  
          
        const isShape = target.type === 'food';  
        const hasGuns = target.guns && target.guns.length > 0;  
          
        // Get close to shapes without guns  
        if (isShape && !hasGuns) {  
            return {  
                x: target.x,  
                y: target.y,  
        ...
        // Sidestep against players  
        const range = this.getIdealCombatRange();  
        const dist = util.getDistance(body, target);  
          
        // Update sidestep timer  
        this.movement.sidestepTimer--;  
        if (this.movement.sidestepTimer <= 0) {  
            this.movement.sidestepAngle = (Math.random() - 0.5) * Math.PI;  
            this.movement.sidestepTimer = 20 + Math.random() * 40;  
        }  
          
        const angle = Math.atan2(target.y - body.y, target.x - body.x);  
        const sidestepDist = this.personality.sidestepping.distance * 50;  
          
        // Calculate sidestep position  
        const sidestepX = Math.cos(angle + this.movement.sidestepAngle) * sidestepDist;  
        const sidestepY = Math.sin(angle + this.movement.sidestepAngle) * sidestepDist;  
          
        // Maintain range while sidestepping  
        const rangeFactor = Math.min(1, range / dist);  
          
        return {  
            x: target.x + sidestepX * rangeFactor,  
            y: target.y + sidestepY * rangeFactor,  
        };  
    }  
      
    calculateWanderGoal() {  
        // Similar to existing wanderAroundMap but with shape shooting  
        const body = this.body;  
          
        if (!this.movement.targetLocation ||   
            util.getDistance(body, this.movement.targetLocation) < 50) {  
            // Pick new location  
            const room = global.gameManager.room;  
        ...
        return this.movement.targetLocation;  
    }  
      
    shouldRecoilBoost() {  
        if (!this.body.guns || this.body.guns.length === 0) return false;  
        if (this.currentTarget) return false;  
        if (this.avoidance.threats.length > 0) return false;  
          
        return Math.random() < 0.1; // 10% chance when idle  
    }  
      
    getRecoilBoostDirection() {  
        // Find gun with highest recoil  
        let maxRecoil = 0;  
        let bestGun = null;  
          
        for (const gun of this.body.guns) {  
            if (gun.settings.recoil > maxRecoil) {  
                maxRecoil = gun.settings.recoil;  
                bestGun = gun;  
            }  
        }  
          
        if (!bestGun) return { x: -1, y: 0 };  
          
        // Face opposite to desired movement direction  
        const movementAngle = this.movement.currentAngle || 0;  
        const opposite = movementAngle + Math.PI;  
          
        return {  
            x: Math.cos(opposite + bestGun.angle) * 100,  
            y: Math.sin(opposite + bestGun.angle) * 100,  
        };  
    }  
      
    activateSillyMode() {  
        this.isSillyMode = true;  
        this.sillyModeTimer = 60 + Math.random() * 180; // 1-4 seconds  
    }  
      
    sillyBehavior() {  
        const behaviors = ['dance', 'stare', 'spin'];  
        const chosen = ran.choose(behaviors);  
          
        if (chosen === 'dance') {  
            // Move in a pattern  
            const t = this.sillyModeTimer;  
            return {  
                goal: {  
                    x: this.body.x + Math.sin(t / 10) * 20,  
                    y: this.body.y + Math.cos(t / 10) * 20,  
                },  
            };  
        } else if (chosen === 'stare') {  
            // Look at nearest player without shooting  
            const nearestFrom = nearest(Array.from(targetableEntities.values()), this.body,   
                e => e.type === 'tank' && e.team !== this.body.team);  
              
            if (nearestFrom) {  
                return {  
                    target: {  
                        x: nearestFrom.x - this.body.x,  
                        y: nearestFrom.y - this.body.y,  
                    },  
                    goal: {  
                        x: this.body.x,  
                        y: this.body.y,  
            ...
            // Spin  
            const angle = (this.sillyModeTimer * 0.1) % (Math.PI * 2);  
            return {  
                target: {  
    ...
    activateMistake() {  
        ...
        this.mistakeTimer = 20 + Math.random() * 60; // Brief mistake  
    }  
      
    updateTimers() {  
        if (this.isSillyMode) {  
            this.sillyModeTimer--;  
            if (this.sillyModeTimer <= 0) {  
                this.isSillyMode = false;  
            }  
        }  
    ...
    getEffectiveRange() {  
        ...
        for (const gun of body.guns || []) {  
            if (gun.canShoot) {  
                const v = gun.getTracking();  
                if (v.range > 0) {  
                    range = Math.max(range, v.range);  
                }  
            }  
        }  
    ...
    getIdealCombatRange() {  
        // Calculate based on gun stats  
        const range = this.getEffectiveRange();  
        const bodySize = this.body.size;  
          
        // Stay at 60-80% of max range for optimal combat  
        return range * (0.6 + Math.random() * 0.2) + bodySize * 3;  
    }  
    ...
    getTrackingSpeed() {  
        ...
        for (const gun of body.guns || []) {  
            if (gun.canShoot) {  
                const v = gun.getTracking();  
                if (v.speed > 0) return v.speed;  
            }  
        }  
          
        return body.topSpeed;  
    }  
      
    // Called when bot takes damage (hook this into damage system)  
    recordDamage(attacker, amount, type) {  
        if (!this.memory.damageFrom.has(attacker.id)) {  
            this.memory.damageFrom.set(attacker.id, {  
                total: 0,  
                lastHit: 0,  
                type: type,  
            });  
        ...
        const record = this.memory.damageFrom.get(attacker.id);  
        record.total += amount;  
        record.lastHit = Date.now();  
        record.type = type;  
          
        // Clean old memories (only keep last 10 seconds)  
        const now = Date.now();  
        for (const [id, data] of this.memory.damageFrom.entries()) {  
            if (now - data.lastHit > 10000) {  
                this.memory.damageFrom.delete(id);  
            }  
        }  
    }  
}  
